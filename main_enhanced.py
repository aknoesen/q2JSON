"""
Q2JSON Main Processing Pipeline with Enhanced Mathematical Consistency Validation
Complete pipeline: JSON Loading → LaTeX Correction → Enhanced Mathematical Validation → Output
"""
...
try:
    ...
    print("ERROR: Enhanced mathematical consistency detector module not found")
    print("Please ensure mathematical_consistency_detector_enhanced.py exists in modules/")
    sys.exit(1)
...
class Q2JSONPipelineEnhanced:
    """
    Enhanced Q2JSON processing pipeline with improved mathematical validation
    Workflow: Load JSON → LaTeX Correction → Enhanced Mathematical Validation → Save → Report
    """
    
    def __init__(self):
        """Initialize the enhanced pipeline components"""
        self.math_detector = MathematicalConsistencyDetectorEnhanced()
    ...
    def process_file(self, input_file: str, output_file: str = None, 
        ...
        """
        Complete processing pipeline for Q2JSON files with enhanced detection
        ...
        if verbose:
            print("Q2JSON Enhanced Processing Pipeline")
            print("=" * 50)
        ...
        # Step 3: Enhanced Mathematical Consistency Check
        math_results = {"status": "skipped"}
        
        if check_math:
            try:
                math_results = self._run_enhanced_mathematical_validation(questions_data, verbose)
            except Exception as e:
                if verbose:
        ...
        if verbose:
            self._print_enhanced_final_report(results)
    ...
    def _run_enhanced_mathematical_validation(self, questions_data: dict, verbose: bool) -> dict:
        """Run enhanced mathematical consistency validation"""
        
        if verbose:
            print("\nStep 2: Enhanced Mathematical Consistency Validation")
            print(f"   Tolerance threshold: {self.math_detector.tolerance_threshold * 100:.1f}%")
    ...
    def _print_enhanced_final_report(self, results: dict):
        """Print enhanced comprehensive final report"""
        
        print("\n" + "=" * 60)
        print("Q2JSON ENHANCED PROCESSING SUMMARY")
        print("=" * 60)
        ...
        # Enhanced mathematical validation results
        ...
        if math_results.get('status') == 'issues_found':
            ...
            print("4. Check for rounding inconsistencies in final answers")
            
            if any(c.severity == "severe" for c in self.math_detector.contradictions_found):
                print("5. PRIORITY: Address severe contradictions immediately")
            
            print("")


def main():
    """Main entry point for enhanced Q2JSON processing"""
    
    parser = argparse.ArgumentParser(
        description="Enhanced Q2JSON Processing Pipeline with Improved Mathematical Validation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python main_enhanced.py input.json                    # Full processing with enhanced math checking
  python main_enhanced.py input.json -o output.json     # Process and save to specific output
  python main_enhanced.py input.json --no-math-check    # Skip mathematical validation
  python main_enhanced.py input.json --no-latex-fix     # Skip LaTeX correction
  python main_enhanced.py input.json -q                 # Quiet mode (minimal output)
        """
    )
    ...
    pipeline = Q2JSONPipelineEnhanced()
