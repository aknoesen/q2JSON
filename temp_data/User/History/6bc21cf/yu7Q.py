...
def show_stage_3():
    ...
    st.write("🔍 **DEBUG: Stage 3 Session State Inspection**")
    st.write(f"🔍 Current stage: {st.session_state.get('current_stage', 'NOT_SET')}")

    # Check for specific keys
    for key in ["questions_data", "validated_questions", "json_data", "processed_questions"]:
        value = st.session_state.get(key, None)
        if value is None:
            st.error(f"❌ Key '{key}' NOT FOUND in session state.")
        else:
            if isinstance(value, list):
                st.success(f"✅ Key '{key}' found: list with {len(value)} items.")
            elif isinstance(value, dict):
                st.success(f"✅ Key '{key}' found: dict with {len(value)} keys.")
            else:
                st.success(f"✅ Key '{key}' found: type {type(value).__name__}, value: {str(value)[:200]}")

    # Show all session state keys with types and sizes
    st.write("🔍 **All session state keys and summary:**")
