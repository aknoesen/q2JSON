...
def render_json_validation():
    ...
    st.write("DEBUG: Starting render_json_validation")
    ...
    st.write(f"DEBUG: process_and_validate_json returned success={success}, questions_data type={type(questions_data)}")
    ...
    if success and questions_data:
        st.write("DEBUG: Taking SUCCESS branch")
        st.write("DEBUG: Calling render_validation_success")
        render_validation_success(questions_data)
        st.write("DEBUG: Calling render_workflow_completion (success path)")
        render_workflow_completion()
    else:
        st.write("DEBUG: Taking FAILURE branch")
        st.write("DEBUG: Validation failed, showing navigation selectors only")
...
def process_and_validate_json():
    ...
    st.write(f"DEBUG: process_raw_json returned success={success}, questions_data type={type(questions_data)}, messages={messages}")

    # Display processing messages
    display_processing_messages(messages)

    # Defensive: Only set questions_data if valid
    if success and questions_data and isinstance(questions_data, dict) and 'questions' in questions_data:
        st.session_state.questions_data = questions_data
        st.write("DEBUG: questions_data stored in session_state")
    else:
...
def render_validation_success(questions_data):
    ...
    st.write(f"DEBUG: render_validation_success called with {len(questions_data['questions']) if questions_data and 'questions' in questions_data else 0} questions")
...
def render_export_download(export_filename, questions_data):
    ...
    # Auto-advance to Stage 3 after validation complete (only on true validation success)
...
def render_workflow_completion():
    ...
    st.write("DEBUG: render_workflow_completion() START")
